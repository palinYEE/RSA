...
void karatsubaMul(unsigned char *a, unsigned char *b, unsigned char *dst, int loopNum){
    ...
    a0b0 = calloc(loopNum, sizeof(unsigned char));
    a1b1 = calloc(loopNum, sizeof(unsigned char));

    a0a1 = calloc(loopNum/2+1, sizeof(unsigned char));
    b0b1 = calloc(loopNum/2+1, sizeof(unsigned char));
    ...
    if(loopNum == 2){
        ...
        memcpy(dst + loopNum/2 + 1, middle, loopNum + 2);
        memcpy(dst + loopNum + 3, a1b1, loopNum/2);
    }
    free(a0b0);
    free(a1b1);
    free(a0a1);
